...
export default function OrderCard({ order }: OrderCardProps) {
   ...
   return (
      <section
         ...
         className="mt-5 rounded-lg bg-gray-50 px-4 py-6 sm:p-6  lg:mt-0 lg:p-8 space-y-4 shadow-2xl"
      >
         ...
         <dl className="mt-6 space-y-4">
            {order.orderProducts.map(product => (
               ...
               >
                  <dt className="flex items-center text-xs text-gray-600">
                     <span className="font-black">({product.quantity}) {''}</span>
                  </dt>
                  <dd className="text-sm font-medium text-gray-800">{product.Product.name}</dd>
               </div>
            ))}
